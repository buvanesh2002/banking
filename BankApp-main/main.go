...
func initApp(mongoClient *mongo.Client){
	ctx = context.TODO()
	profileCollection := mongoClient.Database(constants.DatabaseName).Collection("customer")
...
func initAcc(mongoClient *mongo.Client){
	ctx = context.TODO()
	accCollection := mongoClient.Database(constants.DatabaseName).Collection("account")
...
func initBank(mongoClient *mongo.Client){
	ctx = context.TODO()
	bCollection := mongoClient.Database(constants.DatabaseName).Collection("bank")
...
func initLoan(mongoClient *mongo.Client){
	ctx = context.TODO()
	lCollection := mongoClient.Database(constants.DatabaseName).Collection("loan")
